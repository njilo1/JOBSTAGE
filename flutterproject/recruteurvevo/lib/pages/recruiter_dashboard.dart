...
import 'candidates/candidates_list_page.dart';
...
class _RecruiterDashboardState extends State<RecruiterDashboard> {
  ...
  void _loadData() {
    _entreprise = _dataService.entreprises.first;
    _offres = _dataService.getOffresByEntreprise(_entreprise.id);
    _candidatsRecommandes = _dataService.getCandidatsRecommandes(_offres.first.id);
    _statistiques = _dataService.getStatistiquesEntreprise(_entreprise.id);
  }
      ...
      children: [
        ...
        Row(
          children: [
            ...
            const SizedBox(width: 10),
            _buildHeaderIconWithBadge(Icons.notifications, _getHighMatchCount()),
          ],
        ),
      ],
    );
  ...
  Widget _buildVerificationBadge() {
    if (!_entreprise.isVerified) return const SizedBox.shrink();
          ...
          GridView.count(
            ...
            children: [
              QuickActionCard(
                ...
                title: 'Publier Offre',
                iconColor: AppTheme.customColors['green_dark']!,
                backgroundColor: AppTheme.customColors['green_light']!,
                onTap: () => _navigateToCreateOffer(),
              ),
              QuickActionCard(
                icon: Icons.group,
                title: 'Mes Candidats',
                iconColor: AppTheme.customColors['blue_dark']!,
                backgroundColor: AppTheme.customColors['blue_light']!,
                onTap: () => _navigateToCandidates(),
              ),
              QuickActionCard(
                icon: Icons.psychology,
                title: 'Candidats Matches',
                iconColor: AppTheme.customColors['purple_dark']!,
                backgroundColor: AppTheme.customColors['purple_light']!,
                onTap: () => _navigateToMatches(),
              ),
              QuickActionCard(
                icon: Icons.bookmark_border,
                title: 'Mes Favoris',
                iconColor: AppTheme.customColors['red_dark']!,
                backgroundColor: AppTheme.customColors['red_light']!,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          ..._offres.where((offre) => offre.isActive).map(
                (offre) => OfferCard(
                  offre: offre,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 15),
          ..._candidatsRecommandes.take(3).map(
  ...
  void _navigateToCandidates() {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => const CandidatesListPage(),
      ),
    );
  }
  ...
  void _navigateToCandidatesWithTab() {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => const CandidatesListPage(initialTabIndex: 1),
      ),
    );
  }
  ...
  void _navigateToOfferDetails(Offre offre) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => OfferDetailsPage(offre: offre),
      ),
    );
  }
  ...
  void _showNotifications() {
    ...
    final highMatchCandidates = _dataService.candidats.where((candidat) {
        ...
        return AlertDialog(
          ...
          content: SizedBox(
            ...
            child: highMatchCandidates.isEmpty
                ? const Text('Aucun candidat avec un match de plus de 90% pour le moment.')
                : ListView.builder(
                    shrinkWrap: true,
  ...
  int _getHighMatchCount() {
    return _dataService.candidats.where((candidat) {
      final matchScore = _calculateMatchScore(candidat);
      return matchScore > 90;
    }).length;
  }
  ...
  void _viewCandidateProfile(Candidat candidat) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _buildProfileInfo('Email', candidat.email, Icons.email),
                _buildProfileInfo('Téléphone', candidat.telephone, Icons.phone),
                _buildProfileInfo('Localisation', candidat.localisation, Icons.location_on),
                _buildProfileInfo('Expérience', '${candidat.experiences.length} expérience${candidat.experiences.length > 1 ? 's' : ''}', Icons.work),
                _buildProfileInfo('Éducation', '${candidat.niveauEtude} - ${candidat.universite}', Icons.school),
                const SizedBox(height: 8),
                const Text('Compétences:', style: TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Wrap(
                  children: candidat.competences.map((competence) => Padding(
                    padding: const EdgeInsets.only(right: 4, bottom: 4),
                    child: Chip(
                      label: Text(competence, style: const TextStyle(fontSize: 12)),
                ...
                const SizedBox(height: 4),
                Text('Diplômé(e) en ${candidat.domaineEtude} de ${candidat.universite} en ${candidat.anneeDiplome}. ${candidat.experiences.isNotEmpty ? 'Expériences: ${candidat.experiences.join(", ")}' : 'Recherche une première expérience professionnelle.'}', style: const TextStyle(fontSize: 14)),
              ],
            ),
          ),
          actions: [
...
class _SupportChatPageState extends State<SupportChatPage> {
  ...
  final Map<String, String> _faqDatabase = {
    'comment créer une offre': 'Pour créer une offre d\'emploi, allez dans "Offres" puis cliquez sur "Créer une offre". Remplissez tous les champs requis et publiez votre offre.',
    'comment modifier mon profil': 'Pour modifier votre profil, cliquez sur l\'icône entreprise en haut à droite, puis sur l\'icône crayon pour activer le mode édition.',
    'comment rechercher des candidats': 'Utilisez la section "Candidats" pour rechercher et filtrer les profils selon vos critères.',
    'mot de passe oublié': 'Pour réinitialiser votre mot de passe, allez sur la page de connexion et cliquez sur "Mot de passe oublié".',
    'problème de connexion': 'Vérifiez votre connexion internet et essayez de vous reconnecter. Si le problème persiste, contactez le support.',
    'comment supprimer mon compte': 'Allez dans Paramètres > Confidentialité > Suppression du compte pour supprimer définitivement votre compte.',
    'aide': 'Je suis là pour vous aider ! Posez-moi votre question et je ferai de mon mieux pour vous répondre.',
  ...
  void initState() {
    super.initState();
    _addBotMessage('Bonjour ! Je suis votre assistant virtuel. Comment puis-je vous aider ?');
  }
  ...
  void _addBotMessageWithCopyButton() {
    setState(() {
      _messages.add(ChatMessage(
        text: 'Cliquez sur le bouton ci-dessous pour copier le numéro CENADI :',
        isUser: false,
        timestamp: DateTime.now(),
        hasCopyButton: true,
      ));
  ...
  void _processUserMessage(String message) {
    ...
    if (response != null) {
      ...
      _addBotMessage('Je ne trouve pas de réponse à votre question dans ma base de données. Pour une assistance personnalisée, contactez notre équipe CENADI sur WhatsApp au 676295488.');
  ...
  void _openWhatsApp() {
    ...
    const message = 'Bonjour, j\'ai besoin d\'aide avec l\'application JobStage.';
