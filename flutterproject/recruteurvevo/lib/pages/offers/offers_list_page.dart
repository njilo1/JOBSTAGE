...
class _OffersListPageState extends State<OffersListPage> {
  ...
  void _loadOffres() {
    ...
    Future.delayed(const Duration(milliseconds: 500), () {
      setState(() {
        _offres = List.from(_dataService.offres); // Créer une nouvelle liste pour forcer la mise à jour
        _isLoading = false;
      });
    });
  }

  List<dynamic> get _filteredOffres {
    ...
    return _offres.where((offre) => offre.typeContrat == _selectedFilter).toList();
  }
  ...
  Widget _buildOfferCardWithActions(dynamic offre) {
              ...
              children: [
                        ...
                        children: [
                          Text(
                            ...
                            style: AppTheme.headlineSmall.copyWith(
                              ...
                              color: isDisabled ? Colors.grey[600] : AppTheme.customColors['primary_text'],
                            ),
                            maxLines: 2,
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    if (offre.salaire.isNotEmpty)
                      _buildInfoChip(offre.salaire, Icons.attach_money),
                  ],
                ),
                const SizedBox(height: 12),
  ...
  Widget _buildStatusChip(dynamic offre) {
  ...
  void _toggleOfferStatus(dynamic offre) {
    setState(() {
  ...
  void _showOfferActions(dynamic offre) {
  ...
  void _showDeleteConfirmation(dynamic offre) {
    showDialog(
      context: context,
  ...
  void _deleteOffer(dynamic offre) {
    setState(() {
  ...
  void _navigateToOfferDetails(dynamic offre) {
    if (!offre.isActive) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Cette offre est désactivée. Activez-la pour y accéder.'),
