...
class _NotificationsPageState extends State<NotificationsPage> {
  ...
  void _loadNotifications() {
    setState(() {
      _notifications = _dataService.getNotificationsByDestinataire('1');
    });
  }
  ...
  Widget _buildNotificationCard(NotificationModel notification) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: notification.isLue ? Colors.white : AppTheme.customColors['blue_light']!.withOpacity(0.3),
  ...
  Widget _buildNotificationIcon(String type) {
    ...
    switch (type) {
      ...
      case 'candidat_match':
        ...
        color = AppTheme.customColors['purple_dark']!;
        break;
      case 'offre_expiree':
        icon = Icons.schedule;
        color = AppTheme.customColors['orange_dark']!;
        break;
  ...
  void _markAllAsRead() {
    for (final notification in _notifications) {
      if (!notification.isLue) {
        _dataService.marquerNotificationCommeLue(notification.id);
      }
    }
    _loadNotifications();
  ...
  void _navigateToOffers() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Navigation vers les offres')),
    );
  }
